...
const AUDIT_ACTIONS = [
	"USER_REGISTERED", "USER_LOGGED_IN", "USER_DELETED", "USER_RESTORED",
	"KYC_SUBMITTED", "KYC_APPROVED", "KYC_REJECTED", "KYC_RESUBMITTED",
	"PAYOUT_ACCOUNT_ADDED", "PAYOUT_ACCOUNT_APPROVED", "PAYOUT_ACCOUNT_DEACTIVATED",
	"EVENT_CREATED", "EVENT_SUBMITTED_FOR_REVIEW", "EVENT_APPROVED", "EVENT_REJECTED",
	...
	"ADMIN_HOST_SUSPENDED", "ADMIN_HOST_RESTORED", "ADMIN_PLAN_PRICE_CHANGED",
	"SCANNER_SESSION_CREATED", "SCANNER_SESSION_EXPIRED",
	"TICKET_SCANNED", "DUPLICATE_SCAN_ATTEMPT",
...
function actionColor(action: string): string {
	if (action.includes("APPROVED") || action.includes("CONFIRMED") || action.includes("RESTORED") || action.includes("SCANNED")) {
		return "text-green-700 bg-green-50"
	}
	if (action.includes("REJECTED") || action.includes("CANCELLED") || action.includes("DELETED") || action.includes("DEACTIVATED") || action.includes("SUSPENDED") || action.includes("DUPLICATE")) {
		...
		() => [
			{
				...
				header: "Action",
				cell: ({ row }) => (
					<span className={`inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${actionColor(row.original.action)}`}>
