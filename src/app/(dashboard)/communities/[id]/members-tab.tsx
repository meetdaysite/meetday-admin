...
const QUICK_ACTIONS: { label: string; description: string; icon: LucideIcon; bg: string; color: string }[] = [
	...
	},
	{
		...
		description: "Create and manage member segments",
		icon: Layers,
		bg: "bg-purple-50",
	...
	},
	{
		...
		description: "View detailed activity analytics",
		icon: BarChart2,
		bg: "bg-teal-50",
		color: "text-teal-500",
	},
...
export function MembersTab({ communityId }: { communityId: string }) {
	...
	const filtered = (() => {
		...
		if (q)
			items = items.filter(m => m.name.toLowerCase().includes(q) || m.handle.toLowerCase().includes(q))
			...
			<div className="flex-1 min-w-0 flex flex-col gap-5">
				...
				<div className="grid grid-cols-3 gap-3 lg:grid-cols-5">
					...
					<StatCard
						...
						trend={
							stats ? { value: stats.newMembersGrowth, direction: "up", label: "%" } : undefined
						}
						sub="vs last 30 days"
						accent="sky"
					/>
					<StatCard
						...
						trend={
							stats
								? { value: stats.engagementRateGrowth, direction: "up", label: "%" }
								: undefined
						}
						sub="vs last 30 days"
						accent="amber"
					...
					<StatCard
						...
						trend={
							stats
								? { value: stats.retentionRateGrowth, direction: "up", label: "%" }
								: undefined
						}
						sub="vs last 30 days"
						accent="rose"
			...
			<div className="hidden lg:flex w-72 shrink-0 flex-col gap-4">
				...
				<div className="rounded-xl border border-amber-100 bg-amber-50 p-4">
					...
					<p className="text-[11px] text-amber-700 leading-relaxed">
						Engage your top members by pinning their posts, featuring their content, and inviting
						them to host experiences.
					</p>
					<button
