...
import {
	Crown, Shield, Users, Eye, UserPlus, Search, SlidersHorizontal,
	ChevronRight, ArrowRight, ShieldCheck, LogIn, KeyRound, Puzzle,
	Lock, FileBarChart, Plus, Send, Info, TrendingUp, type LucideIcon,
} from "lucide-react"
...
const ROLE_ICON_MAP: Record<string, LucideIcon> = {
	crown: Crown, shield: Shield, users: Users, eye: Eye,
}

const ACTIVITY_ICON_MAP: Record<string, LucideIcon> = {
	promote: TrendingUp, join: UserPlus, edit: ShieldCheck,
}

const SECURITY_ITEMS = [
	{ label: "Two-Factor Authentication", description: "Required for all managers", icon: ShieldCheck, badge: "Enabled", badgeClass: "bg-green-100 text-green-700" },
	{ label: "Login Activity",            description: "Monitor access and logins",  icon: LogIn,       badge: null,       badgeClass: "" },
	{ label: "Access Requests",           description: "0 pending requests",         icon: KeyRound,    badge: null,       badgeClass: "" },
	{ label: "API & Integrations",        description: "Manage app access",          icon: Puzzle,      badge: null,       badgeClass: "" },
]
...
function RoleOverviewRow({ role }: { role: ManagerRoleOverview }) {
	...
	return (
		<div className="flex items-center gap-3 py-2.5 border-b border-border-subtle last:border-0">
			<div className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-lg", role.bg, role.border, "border")}>
...
export function ManagersTab({ communityId }: { communityId: string }) {
	...
	const load = useCallback(async () => {
		setIsLoading(true)
		setError(null)
		try { setData(await getCommunityManagers(communityId)) }
		...
		<div className="flex items-start gap-5">
			...
			<div className="flex-1 min-w-0 flex flex-col gap-5">
				...
				<div className="flex items-start justify-between gap-4">
					<div>
						<h2 className="text-base font-semibold text-text-primary">Community Managers &amp; Permissions</h2>
						<p className="mt-0.5 text-xs text-text-tertiary">Manage community roles, permissions and access. Add managers, moderators and set role-based access.</p>
					</div>
				...
				<div className="grid grid-cols-2 gap-4">
					...
					<div className="flex flex-col gap-4">
						...
						<div className="rounded-xl border border-border-default bg-surface-card p-4">
							<div className="flex items-center justify-between mb-3">
								...
								<div className="flex items-center gap-2">
									<div className="relative">
										...
										>
											{["All Roles","Owner","Manager","Moderator","View Only"].map(r => <option key={r}>{r}</option>)}
										</select>
										<ChevronRight size={10} className="pointer-events-none absolute right-1.5 top-1/2 -translate-y-1/2 rotate-90 text-text-tertiary" />
									</div>
							...
							{/* Table header */}
							<div className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-x-3 items-center py-1.5 border-b border-border-default text-[10px] font-semibold text-text-tertiary uppercase tracking-wide">
								<span>Member</span>
								<span>Role</span>
							...
							<div className="flex flex-col divide-y divide-border-subtle">
								{filteredMembers.map(m => (
									<div key={m.id} className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-x-3 items-center py-2.5">
										{/* Member */}
										...
										<div className="text-right min-w-[80px]">
											<p className="text-[11px] font-medium text-text-primary whitespace-nowrap">{m.permissionsLabel}</p>
											<p className="text-[10px] text-text-tertiary">{m.permissionsCount} permissions</p>
										</div>
										{/* Joined */}
										<p className="text-[10px] text-text-tertiary whitespace-nowrap">{m.joinedDate}</p>
										{/* Status */}
						...
						<div className="rounded-xl border border-border-default bg-surface-card p-4">
							<div className="mb-3">
								...
								<p className="text-[11px] text-text-tertiary mt-0.5">See what each role can access and manage.</p>
							</div>
								...
								<table className="w-full text-[11px]">
									<thead>
										<tr className="border-b border-border-default">
											<th className="pb-2 text-left font-semibold text-text-secondary">Permissions</th>
											{[
												...
												<th key={col.label} className="pb-2 text-center font-semibold text-text-secondary">
									...
									<tbody>
										{(data?.permissionMatrix ?? []).map(row => (
											<tr key={row.label} className="border-b border-border-subtle last:border-0">
												...
												{[row.owner, row.manager, row.moderator, row.viewOnly].map((has, i) => (
													<td key={i} className="py-2 text-center">
														{has ? (
															<span className="inline-flex items-center justify-center">
																<svg width="14" height="14" viewBox="0 0 14 14" fill="none">
																	<circle cx="7" cy="7" r="7" fill="#dcfce7"/>
																	<path d="M4.5 7L6 8.5L9.5 5" stroke="#16a34a" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
																</svg>
															</span>
														) : (
					...
					<div className="flex flex-col gap-4">
						...
						<div className="rounded-xl border border-border-default bg-surface-card p-4">
							<div className="mb-3">
								<h3 className="text-sm font-semibold text-text-primary">Role &amp; Permission Overview</h3>
								<p className="text-[11px] text-text-tertiary mt-0.5">Manage what each role can access and modify.</p>
							</div>
						...
						<div className="rounded-xl border border-border-default bg-surface-card p-4">
							<div className="mb-3">
								<h3 className="text-sm font-semibold text-text-primary">Invite New Manager</h3>
								<p className="text-[11px] text-text-tertiary mt-0.5">Invite someone to help manage this community.</p>
							</div>
							<div className="flex flex-col gap-3">
								...
								<div className="relative">
									...
									>
										...
										{["Manager","Moderator","View Only"].map(r => <option key={r}>{r}</option>)}
									</select>
									<ChevronRight size={12} className="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 rotate-90 text-text-tertiary" />
								</div>
								<button
									onClick={handleSendInvitation}
									className="flex items-center justify-center gap-2 rounded-lg bg-gradient-to-r from-purple-500 to-indigo-500 py-2.5 text-xs font-semibold text-white hover:from-purple-600 hover:to-indigo-600 transition-all"
								>
					...
					<div className="flex items-center gap-4">
						<div className="relative shrink-0">
							<PieChart width={100} height={100}>
								...
								>
									{(data?.accessSummary ?? []).map((s, i) => <Cell key={i} fill={s.color} />)}
								</Pie>
							</PieChart>
							<div className="absolute inset-0 flex flex-col items-center justify-center">
								<span className="text-sm font-bold text-text-primary">{stats?.totalUsers ?? "—"}</span>
								<span className="text-[9px] text-text-tertiary leading-tight text-center">Total<br/>Users</span>
							</div>
						</div>
						<div className="flex flex-col gap-1.5">
							{(data?.accessSummary ?? []).map(s => (
								<div key={s.label} className="flex items-center gap-2">
									...
									<span className="text-[11px] font-semibold text-text-primary ml-auto pl-2 tabular-nums">{s.count} ({s.pct}%)</span>
								</div>
							))}
						</div>
					</div>
						...
						{SECURITY_ITEMS.map(item => (
							...
							>
								...
								<div className="flex-1 min-w-0">
									...
									<p className="text-[10px] text-text-tertiary truncate">{item.description}</p>
								</div>
								{item.badge ? (
									<span className={cn("shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold", item.badgeClass)}>
										{item.badge}
									</span>
								) : (
