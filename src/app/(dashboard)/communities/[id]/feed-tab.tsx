	...
	return (
		<div className="rounded-xl border border-border-default bg-surface-card p-4 flex flex-col gap-3">
			...
			<div className="flex gap-3">
				{post.imageColor && (
					<div className="h-16 w-24 shrink-0 rounded-lg" style={{ background: post.imageColor }} />
				)}
				<div className="flex-1 min-w-0">
...
export function FeedTab({ communityId }: { communityId: string }) {
	...
	const filtered = posts.filter(p => {
		...
		const matchSearch =
			!q || p.content.toLowerCase().includes(q) || p.authorName.toLowerCase().includes(q)
				...
				<div className="flex items-center gap-2">
					...
					<div className="relative">
						<select
							...
							className="h-8 appearance-none rounded-lg border border-border-default bg-surface-card pl-3 pr-7 text-xs text-text-primary focus:outline-none focus:ring-1 focus:ring-border-focus"
						>
							{(["All Time", "Today", "This Week", "This Month"] as FilterTime[]).map(t => (
								<option key={t}>{t}</option>
							))}
						</select>
						<ChevronDown
					...
					<div className="relative">
						...
						>
							{(
								["All Status", "Queue", "Published", "Rejected", "Pinned"] as FilterStatus[]
							).map(t => (
								<option key={t}>{t}</option>
							))}
						</select>
					...
					<div className="relative ml-auto">
						<select
							...
							className="h-8 appearance-none rounded-lg border border-border-default bg-surface-card pl-3 pr-7 text-xs text-text-primary focus:outline-none focus:ring-1 focus:ring-border-focus"
						>
							{(["Newest First", "Oldest First", "Most Likes", "Most Views"] as SortMode[]).map(
								t => (
									<option key={t}>{t}</option>
								),
							)}
			...
			<div className="hidden lg:flex w-72 shrink-0 flex-col gap-4">
				...
				<div className="rounded-xl border border-purple-100 bg-purple-50 p-4">
					...
					<p className="text-[11px] text-purple-700 leading-relaxed">
						{data?.tip ??
							"Use pinned posts to highlight important updates or community guidelines."}
					</p>
				</div>
			</div>
		</div>
