...
function SponsorshipChatsContent() {
	...
	// Fetch accepted sponsorship/campaign threads
	const sponsorshipThreadsQuery = useQuery({
		queryKey: ["admin-sponsorship-chats", "ACCEPTED"],
		queryFn: () => getSponsorshipChats("ACCEPTED"),
		refetchInterval: THREADS_POLL_MS,
	})

	// Fetch accepted space threads
	const spaceThreadsQuery = useQuery({
	...
	const campaignUnreadCount = allSponsorshipThreads
		.filter((t) => t.type === "CAMPAIGN" || Boolean(t.campaignId) || (!t.proposalId && !t.proposalName && Boolean(t.campaignName)))
		.reduce((sum, t) => sum + (t.unreadCount || 0), 0)
				...
				<div className={cn(
					...
					{activeTab === "SPACES" && (
						<div className="flex border-b border-black/10 md:border-b-[2px] md:border-black/20 bg-neutral-100 shrink-0">
							{(
								[
									{ key: "BRAND", label: "Brand", unread: spacesBrandUnreadCount },
									{ key: "COMMUNITY", label: "Community", unread: spacesCommunityUnreadCount },
								] as const
							).map((sub) => {
						...
						>
							...
							<div
								className={cn(
									...
									m.senderType === "BRAND" && "bg-[#EE2C2C] text-white rounded-br-sm",
									m.senderType === "HOST" && "bg-[#FFC940] text-black rounded-bl-sm",
									(m.senderType as string) === "SPACE" && "bg-black text-white rounded-bl-sm",
						...
						>
							...
							<div
								className={cn(
									...
									isDeleted && "opacity-90",
									m.senderType === "BRAND" && "bg-[#EE2C2C] text-white rounded-br-sm",
									m.senderType === "COMMUNITY" && "bg-[#FFC940] text-black rounded-bl-sm",
								...
								{m.replyTo && (
									<button
										...
										className={cn(
											...
											m.senderType === "ADMIN"
												? "bg-black/25 hover:bg-black/35 text-white border-white/80"
												: m.senderType === "SPACE" || m.senderType === "BRAND"
													? "bg-black/10 hover:bg-black/15 text-black border-black/40"
													: "bg-white hover:bg-neutral-50 text-black border-[#EE2C2C] border border-black/10",
										)}
										title="Click to jump to message"
