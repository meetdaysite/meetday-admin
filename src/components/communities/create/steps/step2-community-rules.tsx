...
import { MessageSquare, Rss, Megaphone, Users, CalendarDays, Shield, AlertTriangle, Link2, Copy, Loader2, Globe, ShieldCheck, Lock } from "lucide-react"
...
const ACCESS_DISPLAY: Record<CommunityType, { label: string; desc: string; icon: React.ReactNode }> = {
	MEETDAY_MANAGED_PUBLIC: { label: "Public Community", desc: "Anyone can discover and join.", icon: <Globe size={16} className="text-[#7c3aed]" /> },
	HOST_LED: { label: "Approval Required", desc: "Members request access and admins approve.", icon: <ShieldCheck size={16} className="text-[#e65100]" /> },
	PRIVATE_INVITE_ONLY: { label: "Invite Only", desc: "Only invited users can join.", icon: <Lock size={16} className="text-[#1565c0]" /> },
}
	...
	return (
		<form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
			...
			<div className="rounded-panel border border-border-subtle bg-surface-canvas p-6 shadow-card">
				...
				<p className="text-caption text-text-secondary mb-4">Determined by the community type selected in Step 1.</p>
			...
			<div className="rounded-panel border border-border-subtle bg-surface-canvas p-6 shadow-card">
				...
				<p className="text-caption text-text-secondary mb-4">Choose the features you want to enable.</p>
				<div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
					{([
						{ name: "chatEnabled" as const, icon: <MessageSquare size={16} className="text-[#7c3aed]" />, bg: "bg-[#f0e6ff]", label: "Community Chat", desc: "Enable chat room for members" },
						{ name: "feedEnabled" as const, icon: <Rss size={16} className="text-[#1565c0]" />, bg: "bg-[#e3f2fd]", label: "Community Feed", desc: "Allow members to create posts" },
						{ name: "announcementsEnabled" as const, icon: <Megaphone size={16} className="text-[#e65100]" />, bg: "bg-[#fff3e0]", label: "Announcements", desc: "Admins & hosts can publish updates" },
						{ name: "memberDirectoryEnabled" as const, icon: <Users size={16} className="text-[#2e7d32]" />, bg: "bg-[#e8f5e9]", label: "Member Directory", desc: "Allow members to browse members" },
						{ name: "experiencesTabEnabled" as const, icon: <CalendarDays size={16} className="text-[#6a1b9a]" />, bg: "bg-[#f3e5f5]", label: "Experiences Tab", desc: "Show upcoming experiences" },
					] as const).map((feat) => (
						<Controller
							key={feat.name}
							name={feat.name}
			...
			<div className="rounded-panel border border-border-subtle bg-surface-canvas p-6 shadow-card">
				<h2 className="text-label-md font-semibold text-text-primary mb-1">3. Posting & Chat Permissions</h2>
				<p className="text-caption text-text-secondary mb-4">Define who can interact in the community.</p>
						...
						render={({ field }) => (
							<PermissionRadioGroup
								...
								options={[
									...
									{ value: "ATTENDED_MEMBERS_ONLY", label: "Attended members only", description: "Ensures quality conversations and reduces spam.", recommended: true },
					...
					<Controller
						...
						render={({ field }) => (
							<PermissionRadioGroup
								...
								options={[
									{ value: "ALL_MEMBERS", label: "All members" },
									{ value: "ATTENDED_MEMBERS_ONLY", label: "Attended members only", description: "Members can chat after attending an event.", recommended: true },
			...
			<div className="rounded-panel border border-border-subtle bg-surface-canvas p-6 shadow-card">
				...
				<div className="grid grid-cols-1 gap-2 sm:grid-cols-2 mb-5">
					{([
						{ name: "spamDetection" as const, label: "Spam Detection", desc: "Detects and filters spam content", icon: <Shield size={14} className="text-[#2e7d32]" /> },
						{ name: "toxicContentDetection" as const, label: "Toxic Content Detection", desc: "Flags harmful and abusive content", icon: <AlertTriangle size={14} className="text-[#e65100]" /> },
						{ name: "linkFiltering" as const, label: "Link Filtering", desc: "Blocks suspicious links", icon: <Link2 size={14} className="text-[#1565c0]" /> },
						{ name: "duplicateContentDetection" as const, label: "Duplicate Content Detection", desc: "Reduces repetitive posts", icon: <Copy size={14} className="text-[#7c3aed]" /> },
					] as const).map((mod) => (
						<Controller
							key={mod.name}
							name={mod.name}
							...
							render={({ field }) => (
								<label className="flex items-start gap-3 cursor-pointer rounded-card border border-border-subtle p-3">
									...
									/>
									<div>
										<p className="text-label-sm font-medium text-text-primary">{mod.label}</p>
				...
				<div className="flex flex-col gap-2">
					...
					<p className="text-caption text-text-secondary">Auto-hide content after N reports by unique members</p>
					<Controller
						...
						render={({ field }) => (
							...
							>
								{REPORT_THRESHOLD_OPTIONS.map((n) => (
									<option key={n} value={n}>{n} reports</option>
								))}
							</select>
						)}
					/>
					...
					<Controller
						...
						render={({ field }) => (
							<PermissionRadioGroup
								...
								onChange={field.onChange}
								options={[
									{ value: "REQUIRE_CONSENT_REMINDER", label: "Require consent reminder", description: "Show reminder before sharing photos." },
